$(function () {
    if ($('.header').hasClass('header--birth')) {
        $('body').addClass('birthdayDay')
    } else {
        $('body').removeClass('birthdayDay')
    }


    $('.header--birth').on('click', function () {
        $(this).toggleClass('header--purple')
    });
    ...
    $('.filter__tag-close').on("click", function () {
        ...
        $(this).closest('.section-tag__block').hide();
    });
    // закрыть теги
    ...
    $('.site-nav__item.search').on("click", function () {
        ...
        $('.suggest-input-search-element').focus();
    });
...
//открытие большого саггеста
    ...
    $('.section-tag__block.section-tag__block--filter').on("click", function () {
        $(this).toggleClass('active')
    });

    $('.section-tag__block.section-tag__block--filter .filter__tag-close').on("click", function () {

        $(this).closest('.section-tag__block').show();
        $(this).parent().show();
    });
    $('.js-bth__ta-swin').on('focus', function () {
        $(this).closest('div').css('height', '100px')
    });
    $('.js-bth__ta-swin').on('blur', function () {
        $(this).closest('div').css('height', '56')
    });
